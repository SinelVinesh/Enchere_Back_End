CREATE OR REPLACE VIEW V_APP_USER AS (
  ...
  (AU.ACCOUNT_BALANCE - SUM(B.AMOUNT)) AS MONEY_CAN_USE FROM BID_HISTORY B JOIN APP_USER AU ON B.APP_USERID = AU.ID WHERE B.ID IN(SELECT MAX(B.ID) FROM BID_HISTORY B GROUP BY B.BIDID) GROUP BY B.APP_USERID,
  AU.ACCOUNT_BALANCE
);
